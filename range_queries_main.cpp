...
//	fastest << cpu_frequencies_array[0];
	...
	default_functions.push_back("range_queries");
	...
	for(uint64_t i = 0; i < max_space_size; i++) {
		search_space[i] = fast_rand(&_seed)% (max_space_size*2);
	}
	std::shuffle(search_space, &search_space[max_space_size], fast_URBG(seed));
	std::map<const std::string, uint64_t(*)(uint64_t *, uint32_t, uint64_t*, uint64_t*, uint32_t)>
		function_map =
		{
#define FUNCTION(n) {#n, n}
			FUNCTION(range_queries_solution),
	...
	uint64_t * query_list_lb = new uint64_t[max_query_count];
	uint64_t * query_list_ub = new uint64_t[max_query_count];

	for(uint i = 0; i < max_query_count; i++) {
		query_list_lb[i] = fast_rand(&_seed)% (max_space_size);
		query_list_ub[i] = query_list_lb[i]+ (fast_rand(&_seed)% (max_space_size));
	}
    char perfstats_header[]="size,rep,function,query_counts,IC,Cycles,CPI,MHz,CT,ET,cmdlineMHz,answer";
...
//		set_cpu_clock_frequency(mhz);
				...
				for(uint r = 0; r < reps; r++) {
					for(auto & function : functions) {
						...
						{								
							...
							answer += function_map[function](search_space, space_size, query_list_lb, query_list_ub, query_count);
							perfstats_disable();
	...
	delete [] search_space;
	delete [] query_list_lb;
	delete [] query_list_ub;
